...
function read_types_from_stores(generate_path_file) {
...
function main() {
  if ( process.argv.includes('-h') ) {
    console.log("Syntax: (npm run main|yarn main|node db/main.js) db/rules/create_rules.js [...options] [--write filename] ")
    console.log("")
    console.log("        --skip-types: Skip loading type definition from model config")
    console.log("        --only-types: Only load type definition from model config")
    console.log("        --gen-paths:  Generate rule paths instead of schema")
    console.log("        --write:      Write bolt definition to file")
    return
  }

  let concat_file_list = [ 'global_types.bolt', 'global_functions.bolt', '<TYPES>', 'rules.bolt' ];

  if ( process.argv.includes('--skip-types') ) {
    delete concat_file_list[2]
  }
  if ( process.argv.includes('--only-types') ) {
    concat_file_list = ['<TYPES>']
  }

  let generate_path_file = process.argv.includes('--gen-paths')
  ...
  concat_file_list.forEach(filename => {
    ...
    if ( filename === '<TYPES>' ) {
      boltFileContent += read_types_from_stores(generate_path_file) + '\n\n';
    } else if ( filename ) {
  ...
  if ( process.argv.includes('--write') ) {
    let target_file = process.argv.pop();
