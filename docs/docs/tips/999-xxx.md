...
- ➡️ Have consistent data validation on client and server
- ➡️ Use an additional layer of abstraction and therefore less vendor lock-in
- (heliosRX will support other backends in the future)
- straight forward CRUD API
- Automatic input validation
- Fully reactive state management — no Vuex needed
- Automatic type conversion for timestamps to moment-js
- Useful extensions of the API, like easy sorting of objects
- 🍭 Elegant abstraction layer for Firebase
...
Is inspired by vuexfire.
although developed independelty, heliosRX has some similarities with vuexORM.
