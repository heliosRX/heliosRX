...
import { parseTpl, analyzeTpl } from './template/utils'
import { rtdbBindAsArray,
         rtdbBindAsObject,
         rtdbFetchAsArray,
         rtdbFetchAsObject } from './vuefire/rtdb'
  ...
  _bind_firestore( { state, commit, key, ref, ops } ) {
    ...
    return new Promise((resolve, reject) => {
      const payload = {
        vm:         state,
        key:        key,
        collection: ref, // FALSCH??
        ops:        ops,
        resolve:    resolve,
        reject:     reject
      };
      sub[key] = ref.where
        ? bindCollection( payload, options )
        : bindDocument( payload, options )
    })
  }
...
// let syncId2 = task_session_list.sync_add(target, { taskId: '234' })
// task_session_list.sync_rem(syncId1)
// sync_add(target, idDataOrId) { /* sync_add(target, { taskId: '123' }) */ }
